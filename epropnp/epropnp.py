...
class EProPnPBase(torch.nn.Module, metaclass=ABCMeta):
    ...
    def monte_carlo_forward(self, x3d, x2d, w2d, camera, cost_fun,
                            pose_init=None, force_init_solve=True, sample_conv_min_extrict=None, **kwargs):
        """
        ...
        # ================== 修改：强制采样半径不小于扰动半径 ==================
        if self.training and pose_cov is not None and sample_conv_min_extrict is not None:
            if len(sample_conv_min_extrict) == 4:
                B = pose_cov.shape[0]
                min_std_x, min_std_y, min_std_z, min_std_r = sample_conv_min_extrict
                min_var = torch.tensor(
                    [min_std_x**2, min_std_y**2, min_std_z**2, 
                    min_std_r**2, min_std_r**2, min_std_r**2], 
                    device=pose_cov.device, dtype=pose_cov.dtype
                )
                eye_inflation = torch.diag(min_var).unsqueeze(0).expand(B, -1, -1)
                pose_cov = pose_cov + eye_inflation
            else:
                raise IndexError(f'Wrong in {sample_conv_min_extrict=:}')
