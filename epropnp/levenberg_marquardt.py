    ...
    def solve(self, x3d, x2d, w2d, camera, cost_fun, **kwargs):
        with torch.no_grad():
            ...
            if bs > 0:
                ...
                # probs = mean_weight.reshape(-1, pn)
                # if torch.isnan(probs).any() or torch.isinf(probs).any() or (probs < 0).any() or probs.sum() <= 0:
                #     print("\n[EPro-PnP Debug] Detected invalid probs for multinomial!")
                #     print(f"probs min: {probs.min()}, max: {probs.max()}, sum: {probs.sum()}")
                #     # 检查是否有特定的行（batch item）全为 0
                #     row_sums = probs.reshape(probs.shape[0], -1).sum(dim=-1)
                #     print(f"Row sums min: {row_sums.min()}")
