...
def set_train_phase(net, phase):
    ...
    if phase == 1:
        ...
        # for param in model.net.hcce_head.parameters():
        #     param.requires_grad = True
        for param in model.net.uncertainty_head.parameters():
            param.requires_grad = True
        for param in model.net.scale_branch.parameters():
            param.requires_grad = True
...
if __name__ == '__main__':
    ...
    # 总训练批次。
    total_epochs = [60, 100]
    # TODO: 若第一阶段epoch达不到target_iteration则按比例扩充epochs
    ...
    if len(total_epochs) == 2:
        epro_start_epoch = total_epochs[0]
    ...
    # start_perturb_epoch = total_epochs[1]
    ...
    for obj_id in obj_id_list:
        ...
        if args.local_rank == 0:
            ...
            if output_save:
                output_save_path_base = os.path.join(output_save, 'obj_%s'%str(obj_id).rjust(2, '0'))
        ...
        if isinstance(total_epochs, list) and len(total_epochs) >= 1:
            planned_phase1_iters = total_epochs[0] * iter_per_epoch
            if planned_phase1_iters < target_iteration:
                adjusted_phase1_epochs = math.ceil(target_iteration / iter_per_epoch)
                adjusted_phase2_epochs = math.ceil(adjusted_phase1_epochs / total_epochs[0] * total_epochs[1])
                total_epochs = [adjusted_phase1_epochs, adjusted_phase2_epochs]
                if local_rank == 0:
                    print(f'Adjust total epoch to make sure iteration more than {target_iteration}: \n New epochs are {total_epochs}.')
        ...
        if epro_start_epoch <= 0:
            ...
            #     optimizer, 
            #     T_max=(phase2_epochs - phase1_epochs - warmup_epochs) * iter_per_epoch,
            #     eta_min=1e-6)
            sch4 = torch.optim.lr_scheduler.LinearLR(
                optimizer, 
                start_factor=0.1, 
                end_factor=0.01,
                total_iters=(phase2_epochs - phase1_epochs - warmup_epochs) * iter_per_epoch)
            ...
            try:
                ...
                if 'scheduler_state_dict' in checkpoint_info and checkpoint_info['scheduler_state_dict']:
                    ...
                    for _ in range(iteration_step - 1):
                        scheduler.step()
                    print(f"Successfully resumed scheduler from step {iteration_step}")
                if start_epoch < total_epochs[0]:
                    set_train_phase(net, 1)
                    phase_dir = "phase1"
                    ...
                    if args.local_rank == 0 and output_save:
                        ...
                        os.makedirs(output_best_save_path, exist_ok=True)
                elif start_epoch < total_epochs[1]:
                    set_train_phase(net, 2)
                    phase_dir = "phase2"
                    current_phase_best_score = 0.0
                    if args.local_rank == 0 and output_save:
                        output_save_path = os.path.join(output_save_path_base, phase_dir)
        ...
        if load_breakpoint and validate_after_load:
            if start_epoch < total_epochs[0] or epro_start_epoch < 0:
                if args.local_rank == 0: 
            ...
            for batch_idx, (rgb_c, mask_vis_c, GT_Front_hcce, GT_Back_hcce, bbox, cam_K, cam_K_B, cam_R_m2c, cam_t_m2c) in enumerate(train_loader):
                ...
                try:
                    with torch.autocast(device_type='cuda', dtype=torch.float16):
                        ...
                        if epoch < total_epochs[0]: # stage 1
                ...
                if args.local_rank == 0:
                    if batch_idx % log_interval == 0 and (logs_done_this_epoch < log_freq_per_epoch - 1) or batch_idx == num_batches - 1:
                        if output_save:
                            ...
                            if epoch < total_epochs[0]:
                                writer.add_scalar('Train/Loss_Total', loss.item(), record_epoch)
                            else:
                                writer.add_scalar('Train/Loss_Total_EPro', loss.item(), record_epoch)
                            ...
                            # 记录采样标准差
                            # if 'monitor' in current_loss.keys():
                    ...
                    # 实时显示Loss到进度条后缀
                    pbar.set_postfix({
                        # "lB": f"{loss_factors['Back_L1Losses'] * torch.sum(current_loss['Back_L1Losses']).item():.4f}",
                        # "lF": f"{loss_factors['Front_L1Losses'] * torch.sum(current_loss['Front_L1Losses']).item():.4f}",
                        # "lM": f"{loss_factors['mask_loss'] * current_loss['mask_loss'].item():.4f}",
                        # "lC": f"{loss_factors['mc_loss'] * current_loss['mc_loss'].item():.4f}",
                        # "lT": f"{loss_factors['t_loss'] * current_loss['t_loss'].item():.4f}",
                        # "lR": f"{loss_factors['r_loss'] * current_loss['r_loss'].item():.4f}",
                        "Tot": f"{loss.item():.4f}"
                    })
