...
import yaml
from HccePose.bop_loader import BopDataset, TrainBopDatasetBackFront, TestBopDatasetBackFront
...
def test(obj_ply, obj_info, net: HccePose_BF_Net, test_loader: torch.utils.data.DataLoader, local_rank, world_size):
    net.eval()
    local_add_list = []
    disable_tqdm = (local_rank != 0)
    for batch_idx, (rgb_c, mask_vis_c, GT_Front_hcce, GT_Back_hcce, Bbox, cam_K, cam_R_m2c, cam_t_m2c) in tqdm(
...
if __name__ == '__main__':
    ...
    train_bop_dataset_back_front_item = TrainBopDatasetBackFront(bop_dataset_item, train_folder_name, padding_ratio=padding_ratio)
    
    # ratio = 0.01 means selecting 1% of samples from the dataset for testing.
    # ratio = 0.01 表示从数据集中选择 1% 的样本作为测试数据。
    test_bop_dataset_back_front_item = TestBopDatasetBackFront(bop_dataset_item, val_folder_name, padding_ratio=padding_ratio, ratio=0.5)
            ...
            for batch_idx, (rgb_c, mask_vis_c, GT_Front_hcce, GT_Back_hcce) in enumerate(train_loader):
                ...
                if (iteration_step % log_freq == 0 and iteration_step > 0):
                    ...
                    if iteration_step % log_freq_a == 0:
                        ...
                        max_acc_id, max_acc, add_list_l = test(obj_ply, obj_info, net_test, test_loader, local_rank=args.local_rank, world_size=world_size if not ide_debug else 1)
                    if args.local_rank == 0:
                        # if isinstance(net, torch.nn.parallel.DataParallel):
                        #     state_dict = net.module.state_dict()
                        # elif isinstance(net, torch.nn.parallel.DistributedDataParallel):
                        ...
                        if iteration_step % log_freq_a == 0:
                            ...
                            if max_acc >= best_score:
                                best_score = max_acc
                                save_best_checkpoint(best_save_path, net, optimizer, best_score, iteration_step, scheduler=scheduler, keypoints_ = add_list_l)
                                if output_save:
                                    save_best_checkpoint(output_best_save_path, net, optimizer, best_score, iteration_step, scheduler=scheduler, keypoints_ = add_list_l)
                ...
                if args.local_rank == 0:
                    ...
                    # 实时显示Loss到进度条后缀
                    pbar.set_postfix({
                        "Front_loss": f"{torch.sum(current_loss['Front_L1Losses']).item():.4f}",
                        "Back_loss": f"{torch.sum(current_loss['Back_L1Losses']).item():.4f}",
                        "Mask_loss": f"{current_loss['mask_loss'].item():.4f}",
                        "Tot": f"{loss.item():.4f}"
                    })
