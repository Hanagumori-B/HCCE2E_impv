...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, query, memory):
        ...
        m_norm = self.norm1(memory)
        
        attn_out, _ = self.cross_attn(query=q_norm, key=m_norm, value=m_norm)
...
class QueryPoseDecoder(nn.Module):
    def __init__(self, feat_dim, rot_dim, num_heads=8, query_nums=1):
        super().__init__()
        self.query_nums = query_nums
        self.pose_queries = nn.Parameter(torch.randn(3, feat_dim))
        self.query_layers = nn.ModuleList([CrossAttentionLayer(feat_dim, num_heads) for i in range(self.query_nums)]
        )
        # 6D Rotation
    ...
    def forward(self, attn_tokens): 
        if self.query_nums == 1:
            if not isinstance(attn_tokens, list):
                B = attn_tokens.shape[0]
                attn_tokens = [attn_tokens]
            else:
                B = attn_tokens[0].shape[0]
        else:
            assert len(attn_tokens) == self.query_nums
            B = attn_tokens[0].shape[0]
        
        queries = self.pose_queries.unsqueeze(0).expand(B ,-1, -1) # [B, 3, 128]
        for i in range(self.query_nums):
            queries = self.query_layers[i](queries, attn_tokens[i])
        
        rot_6d = self.fc_r(queries[:, 0, :])
        xy_site = self.fc_xy(queries[:, 1, :])
        z_site = self.fc_z(queries[:, 2, :])
...
class MultiScaleFeatureNet(nn.Module):
    def __init__(self, 
        ...
        # Input 128x128, depth 4
        super().__init__()
        if out_layers is None:
            self.out_layers=[4]
        else:
            for i in out_layers:
                assert i in [i for i in range(1, 5)]
            self.out_layers = out_layers
        self.out_layer_keys = [f'layer{i}' for i in self.out_layers]
        # 128x128 -> 64x64
        self.layers = nn.ModuleDict({
            'layer1': nn.Sequential(
                nn.Conv2d(in_channels, feat_dim, 3, stride=2, padding=1),
    ...
    def forward(self, x):
        ...
        for k, layer in self.layers.items():
            x = layer(x)
            if k in self.out_layer_keys:
                out_features[k] = self.extra_layers[k](x)
        return [out_features[k] for k in self.out_layer_keys]
...
class PatchPnPNet(nn.Module):
    def __init__(self, 
        ...
        self.features = MultiScaleFeatureNet(in_channels, feat_dim, num_gn_groups, self.feat_layers)
        ...
        # self.fc_xy = nn.Linear(feat_dim, 2)
        # self.fc_z = nn.Linear(feat_dim, 1)
        self.query_pose = QueryPoseDecoder(feat_dim, rot_dim, query_nums=len(self.feat_layers))
    ...
    def forward(self, x: torch.Tensor, region=None, extents=None, mask_attention=None):
        ...
        for feat in feat_list:
            ...
            feat_seq = feat.view(B, C, H * W).permute(0, 2, 1)
            feat_seq = self.pnp_transformer(feat_seq, pos_embed)
            # masked_x = x * avg_mask
            # x = masked_x.sum(dim=1) / (avg_mask.sum(dim=1) + 1e-6) # Masked Pooling
            # x = x.mean(dim=1) # Mean Pooling
            attn_tokens_list.append(feat_seq)
        rot_6d, t_site = self.query_pose(attn_tokens_list)
        return rot_6d, t_site
