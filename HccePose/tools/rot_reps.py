...
def quat2mat_batch(quaternion):
    ...
    elif len(quaternion.shape) == 1:
        quat = F.normalize(quaternion, p=2, dim=-1)
        qw = quat[..., 0]
        qx = quat[..., 1]
        qy = quat[..., 2]
        qz = quat[..., 3]
    ...
    if len(quaternion.shape) == 2:
        ...
        row0 = torch.stack((1 - 2 * yy - 2 * zz, 2 * xy - 2 * zw, 2 * xz + 2 * yw))
        row1 = torch.stack((2 * xy + 2 * zw, 1 - 2 * xx - 2 * zz, 2 * yz - 2 * xw)) 
        row2 = torch.stack((2 * xz - 2 * yw, 2 * yz + 2 * xw, 1 - 2 * xx - 2 * yy))
...
def get_closest_rot_batch(pred_rots, gt_rots, sym_infos):
    ...
    if pred_rots.shape[-1] == 4:
        pred_rots = quat2mat_batch(pred_rots[:, :4])
    if gt_rots.shape[-1] == 4:
        gt_rots = quat2mat_batch(gt_rots[:, :4])
...
#     # test_mat2quat_torch()
#     # test_mat2rot6d_np()
    print(mat2quat_batch(torch.eye(3)))
    print(mat2quat_batch(torch.stack([torch.eye(3), torch.eye(3)])))
    print(quat2mat_batch(torch.tensor([1.,0.,0.,0.])))
    print(quat2mat_batch(torch.stack([torch.tensor([1.,0.,0.,0.]), torch.tensor([1.,0.,0.,0.])])))
