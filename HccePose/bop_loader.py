...
def aug_square_fp32(GT_Bbox, img_sz, dzi_scale_ratio=0.25, dzi_shift_ratio=0.25, padding_ratio=1.5):
    ...
    height = GT_Bbox[3]
    im_H, im_W = img_sz[:2]
    scale_ratio = 1 + dzi_scale_ratio * (2 * np.random.random_sample() - 1)
    shift_ratio = dzi_shift_ratio * (2 * np.random.random_sample(2) - 1) 
    bbox_center = np.array([center_x + width * shift_ratio[0], center_y + height * shift_ratio[1]]) 
    w = max(width, height) * scale_ratio * padding_ratio
    w = min(w, max(im_H, im_W)) * 1.0
...
class RenderingBopDatasetBackFront(Dataset):
    '''
    ---
    ---
    ...
    def __init__(self, bop_dataset_item : BopDataset, folder_name):
        '''
        ---
        ---
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(front_label_image_path) and os.path.exists(back_label_image_path):
            # 这是一个简单的优化：如果文件存在，读取图片大小（或者直接跳过不读），
            # 这里的 return 1 会让 DataLoader 认为这个样本处理完了
            return 1
    ...
    def modified_sym_Rt(self, rot_pose, tra_pose, model_info, e_rot=None):
        ...
        raw_symmetries = model_info['symmetries_discrete']
        for sym in raw_symmetries:
    ...
    def worker_init_fn(self, worker_id):
        ...
        print(f'{worker_id}', end=': ')
        self.worker_id = worker_id
        self.img_shape = cv2.imread(self.dataset_info['obj_info']['obj_%s'%str(self.current_obj_id).rjust(6, '0')][0]['rgb']).shape[:2]
        self.renderer_vispy = renderer.create_renderer(self.img_shape[1], self.img_shape[0], 'vispy_v2', mode='depth', shading='flat')
...
class TrainBopDatasetBackFront(Dataset):
    '''
    ...
    def __init__(self, bop_dataset_item : BopDataset, folder_name, padding_ratio=1.5, crop_size_img=256, aug_op = 'imgaug', ):
        '''
        ---
        ---
        Initialization function.  
...
class TestBopDatasetBackFront(Dataset):
    ...
    def __init__(self, bop_dataset_item : BopDataset, folder_name, bbox_2D = None, test_targets_bop19=None, bbox_2D_score_threshold = 0.0, padding_ratio=1.5, crop_size_img=256, ratio = 1.0 ):
...
class TrainBopDatasetBFEPro(TrainBopDatasetBackFront):
    ...
    def __getitem__(self, index):
        ...
        cam_K_B = cam_K.copy()
        cam_K_B[0, 0] *= bscale  # fx
        cam_K_B[1, 1] *= bscale  # fy
        cam_K_B[0, 2] = (cam_K[0, 2] - bX) * bscale  # cx
        cam_K_B[1, 2] = (cam_K[1, 2] - bY) * bscale   # cy
        ...
        return rgb_c, mask_vis_c, GT_Front_hcce, GT_Back_hcce, Bbox_tensor, cam_K_tensor, cam_K_B_tensor, cam_R_tensor, cam_t_tensor


class TestBopDatasetBFEPro(TestBopDatasetBackFront):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
