...
if __name__ == '__main__':
    ...
    for obj_id in range(start_obj_id, end_obj_id + 1):
        ...
        if epro_start_epoch <= 0:
            ...
            milestones = [warmup_epochs * iter_per_epoch, epro_start_epoch * iter_per_epoch, epro_cos_epoch * iter_per_epoch]

            # 1. 预热段 (0.01 * base_lr -> base_lr)
            sch1 = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.01, end_factor=1.0, total_iters=warmup_epochs * iter_per_epoch)

            # 2. 等待段 (保持 base_lr)
            sch2 = torch.optim.lr_scheduler.ConstantLR(optimizer, factor=1.0, total_iters=(epro_start_epoch - warmup_epochs) * iter_per_epoch)

            sch3 = torch.optim.lr_scheduler.LinearLR(optimizer, start_factor=0.01, end_factor=1.0, total_iters=(epro_cos_epoch - epro_start_epoch) * iter_per_epoch)
            # 3. EPro 段 (从 base_lr * 0.1 开始骤降，然后余弦退火到 1e-6)
            # 注意：这里 factor=0.1 实现了你想要的“手动降速”
            sch4 = torch.optim.lr_scheduler.CosineAnnealingLR(
                optimizer, 
                T_max=(total_epochs - epro_cos_epoch) * iter_per_epoch, 
                eta_min=1e-6
            )
        ...
        for epoch in range(start_epoch, total_epochs):
            ...
            if args.local_rank == 0 and output_save: 
                writer.add_scalar('Test/ADD-S_Accuracy', max_acc, iteration_step)
                writer.add_scalar('Test/Loss_Monte_Carlo', mc_loss.item(), iteration_step)
